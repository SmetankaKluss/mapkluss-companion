...
function Find-JetBrainsRuntime([int]$requiredVersion) {
    ...
    if ($env:MAPKLUSS_LIBRARY_HARNESS_JAVA_HOME) { $candidates.Add($env:MAPKLUSS_LIBRARY_HARNESS_JAVA_HOME) }
    $roots = @(
    ...
    foreach ($candidateHome in $candidates | Select-Object -Unique) {
        ...
        if ($contents -notmatch 'JetBrains') { continue }
        if ($contents -match 'JAVA_VERSION="(?<major>\d+)') {
            if ([int]$Matches.major -eq $requiredVersion) { return $candidateHome }
        }
    }
    return $null
}
...
if (-not $javaHome) {
    throw "JetBrains Runtime $javaVersion was not found. Set MAPKLUSS_LIBRARY_HARNESS_JAVA_HOME to its home directory."
}
...
$common = @(
    ...
    '-Pmapkluss_normal_library_harness=true',
    "-Pmapkluss_library_fixture=$Fixture",
    "-Pmapkluss_library_language=$Language",
    "-Pmapkluss_library_harness_java_home=$javaHome",
    "-Pmapkluss_library_harness_java_executable=$java"
)

Write-Host "MapKluss production Library harness: $MinecraftVersion / $Fixture / $Language" -ForegroundColor Green
Write-Host "This opens the ordinary Library screen with only local anonymous fixture data." -ForegroundColor DarkGray
...
$hotSwapClasses = Join-Path $repo 'build\library-harness\hotswap'
$clientClasses = Join-Path $repo 'build\classes\java\main'
New-Item -ItemType Directory -Force -Path $hotSwapClasses | Out-Null
& $javac --add-modules jdk.jdi -d $hotSwapClasses $hotSwapSource
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
...
try {
    ...
    $jobs.Add($client)
    $hotSwap = Start-Process -FilePath $java -ArgumentList @(
        '--add-modules', 'jdk.jdi', '-cp', $hotSwapClasses, 'MapKlussHotSwap', '5005', $clientClasses
    ) -PassThru -NoNewWindow
    $jobs.Add($hotSwap)
    $compiler = Start-Process -FilePath '.\gradlew.bat' -ArgumentList @(
        'compileJava', '--continuous', "-Pminecraft_version=$MinecraftVersion", '-Pmapkluss_normal_library_harness=true',
        "-Pmapkluss_library_fixture=$Fixture", "-Pmapkluss_library_language=$Language"
    ) -PassThru -NoNewWindow
    $jobs.Add($compiler)
    $client.WaitForExit()
