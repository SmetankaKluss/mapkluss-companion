...
function Find-JetBrainsRuntime([int]$requiredVersion) {
    ...
    if ($env:MAPKLUSS_UI_LAB_JAVA_HOME) { $candidates.Add($env:MAPKLUSS_UI_LAB_JAVA_HOME) }
    $roots = @(
...
if (-not $javaHome) {
    ...
    & .\gradlew.bat compileJava "-Pminecraft_version=$MinecraftVersion" -Pmapkluss_ui_lab=true
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
    $javaHome = Find-JetBrainsRuntime $javaVersion
}
if (-not $javaHome) {
    throw "JetBrains Runtime $javaVersion was not found. Set MAPKLUSS_UI_LAB_JAVA_HOME to its home directory."
}

$java = Join-Path $javaHome 'bin\java.exe'
$javac = Join-Path $javaHome 'bin\javac.exe'
$capture = if ($CaptureOnOpen) { 'true' } else { 'false' }
$common = @(
    'runClient',
    "-Pminecraft_version=$MinecraftVersion",
    '-Pmapkluss_ui_lab=true',
    "-Pmapkluss_ui_lab_java_home=$javaHome",
    "-Pmapkluss_ui_lab_java_executable=$java",
    "-Pmapkluss_ui_lab_capture_on_open=$capture"
)

Write-Host "MapKluss UI Lab $MinecraftVersion / JBR $javaVersion" -ForegroundColor Green
Write-Host "Resources reload automatically. F8 toggles the lab." -ForegroundColor DarkGray
...
$hotSwapClasses = Join-Path $repo 'build\ui-lab\hotswap'
...
try {
    ...
    $compiler = Start-Process -FilePath '.\gradlew.bat' -ArgumentList @(
        'compileJava', '--continuous', "-Pminecraft_version=$MinecraftVersion", '-Pmapkluss_ui_lab=true'
    ) -PassThru -NoNewWindow
    $jobs.Add($compiler)
    $client.WaitForExit()
