...
final class WorkshopDraw {
    ...
    static void icon(DrawContext g, WorkshopIcon icon, int x, int y, int color) {
        g.drawTexture(RenderPipelines.GUI_TEXTURED,ICONS,x,y,(float)icon.u(),0f,16,16,16,16,WorkshopIcon.atlasWidth(),16,color);
    }
    static void image(DrawContext g, Identifier image, WorkshopLayout.Rect area, int iw, int ih) {
        ...
        g.drawTexture(RenderPipelines.GUI_TEXTURED,image,r.x(),r.y(),0f,0f,r.width(),r.height(),iw,ih,iw,ih);
    }
}
