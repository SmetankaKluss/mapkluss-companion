...
final class LensTextureAtlas implements AutoCloseable {
    ...
    private void install(NativeImage image, PendingRequest request) {
        ...
        Identifier nextIdentifier = Identifier.fromNamespaceAndPath(
        ...
        try {
            client.getTextureManager().register(nextIdentifier, texture);
        ...
        if (previous != null) client.getTextureManager().release(previous);
    }
    ...
    public void close() {
        ...
        if (current != null) Minecraft.getInstance().getTextureManager().release(current);
