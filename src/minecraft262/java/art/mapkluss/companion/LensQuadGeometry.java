...
final class LensQuadGeometry {
    ...
    static Quad quad(BlockPos block, Direction facing) {
        ...
        Vec3 right = switch (facing) {
            ...
            case UP -> new Vec3(HALF_CELL, 0, 0);
            case DOWN -> new Vec3(-HALF_CELL, 0, 0);
        };
        Vec3 up = switch (facing) {
            case NORTH, SOUTH, EAST, WEST -> new Vec3(0, HALF_CELL, 0);
            case UP, DOWN -> new Vec3(0, 0, -HALF_CELL);
        };
        return new Quad(
