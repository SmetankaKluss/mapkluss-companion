    ...
    static final class PreviewTexture {
        ...
        synchronized void start() {
            ...
            CompletableFuture.supplyAsync(() -> download(requestedUrl), DOWNLOADS)
                .thenAccept(image -> Minecraft.getInstance().execute(() -> register(image)))
                .exceptionally(error -> {
        ...
        private void register(NativeImage image) {
            try {
                ...
                identifier = Identifier.fromNamespaceAndPath("mapkluss-companion", "preview/" + Integer.toUnsignedString(cacheKey.hashCode(), 16));
                Minecraft.getInstance().getTextureManager().register(
                    identifier,
                    new DynamicTexture(() -> "MapKluss cloud preview", image)
                );
                failed = false;
            } finally {
                loading = false;
