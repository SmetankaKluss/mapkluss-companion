...
public final class MapStackSlotRenderer {
    ...
    public static void draw(GuiGraphicsExtractor context, ItemStack stack, int x, int y) {
        Minecraft client = Minecraft.getInstance();
        if (client.level == null || stack == null || stack.isEmpty()) return;
        Optional<MapPreviewStore.Entry> preview = MapStackManager.instance().preview(client, stack);
        preview.ifPresent(entry -> context.blit(
    ...
    private static synchronized Identifier texture(Minecraft client, String hash, byte[] colors) {
        ...
        for (int pixel = 0; pixel < colors.length; pixel++) {
            ...
            int argb = colorByte == 0 ? 0xFFB8A678 : MapColor.getColorFromPackedId(colorByte);
            image.setPixel(pixel % MAP_SIZE, pixel / MAP_SIZE, argb);
        }
        ...
        client.getTextureManager().register(
            identifier,
            new DynamicTexture(() -> "MapKluss MapStack preview", image)
        );
    ...
    private static void trim(Minecraft client) {
        while (TEXTURES.size() > MAX_TEXTURES) {
            ...
            iterator.remove();
            client.getTextureManager().release(oldest.getValue());
        }
    }

    private static void drawBorder(GuiGraphicsExtractor context, int x, int y, int color) {
    ...
    private static void drawTinyNumber(GuiGraphicsExtractor context, int x, int y, int tileNumber, int color) {
