...
final class CompanionI18n {
    ...
    static {
        ...
        put("Найти", "Find");
        put("Очистить", "Clear");
        put("Очистить поиск", "Clear search");
        put("Сброс", "Reset");
        ...
        put("Проверить обновление", "Check for updates");
        put("Проверяю обновления...", "Checking for updates...");
        put("Установлена актуальная версия.", "The latest version is installed.");
        ...
        put("Аккаунт", "Account");
        put("Детали", "Details");
        put("Скрыть детали", "Hide details");
        put("Cloud подключён", "Cloud connected");
        put("Вход не выполнен", "Not signed in");
        put("Библиотека, Lens и прогресс синхронизируются", "Library, Lens and progress are synchronized");
        put("Войдите через код на сайте MapKluss", "Sign in with a code on the MapKluss website");
        put("Сохранено: ", "Saved: ");
        put("Истекает: ", "Expires: ");
        put("Делиться анонимной статистикой", "Share anonymous usage statistics");
        put("Помочь улучшать мод анонимной статистикой?", "Help improve the mod with anonymous usage statistics?");
        put("Без аккаунта, серверов, артов и координат.", "No account, servers, arts, or coordinates.");
        put("Анонимная статистика: вкл.", "Anonymous statistics: on");
        put("Анонимная статистика: выкл.", "Anonymous statistics: off");
        put("Анонимная статистика включена.", "Anonymous statistics enabled.");
        put("Анонимная статистика отключена.", "Anonymous statistics disabled.");
        put("Не удалось сохранить выбор.", "Could not save the choice.");
        put("Lens не подключён", "Lens is not connected");
        put("Ожидаю live-сессию редактора", "Waiting for a live editor session");
        put("Запусти Lens в редакторе или войди по коду", "Start Lens in the editor or join by code");
        put("Выбери сессию и закрепи её по угловой рамке", "Select a session and anchor it from a corner frame");
        put(" зр.  ·  ", " viewers  ·  ");
        put("px/карта", "px/map");
        put("Сканов пока нет", "No scans yet");
        put("Выбери карту в руке, рамку или стену с картами", "Choose a map in hand, a frame, or a wall of maps");
        put("Недавние сессии", "Recent sessions");
        put("Открой трекер из сохранённого арта или вставь UUID", "Open the tracker from a saved art or paste a UUID");
        put("Источник", "Source");
        put("План строительства", "Build plan");
        ...
        put("Закрепить по рамке", "Place on frame");
        put("Режим: ", "Mode: ");
        ...
        put("Файлы и схемы", "Files and schematics");
        put("Облачный арт", "Cloud art");
        put("Инструменты постройки", "Build tools");
        put("Экспорт и архивы", "Exports and archives");
        put("Облако готовит изображение арта", "Cloud is preparing the art preview");
        put("Обновление...", "Refreshing...");
    ...
    static boolean english(Minecraft client) {
    ...
    private static String language(Minecraft client) {
        ...
        try {
            cachedLanguage = CompanionConfig.load(client.gameDirectory.toPath()).language();
    ...
    static String toggleLabel(Minecraft client) {
        return english(client) ? "RU" : "EN";
    }

    static void toggle(Minecraft client) throws IOException {
        CompanionConfig config = CompanionConfig.load(client.gameDirectory.toPath());
        ...
        new CompanionConfig(config.supabaseUrl(), config.supabaseAnonKey(), config.siteUrl(), next, config.gatewayUrl())
            .saveForRunDir(client.gameDirectory.toPath());
        cachedLanguage = next;
    }

    static Component text(String value) {
        return Component.literal(translate(value));
    }

    static String translate(String value) {
        return translate(value, english(Minecraft.getInstance()));
    }
