...
public final class LiveBuildScreen extends Screen {
    ...
    public LiveBuildScreen(Screen parent,String artId,String versionId) {
        super(Component.literal("MapKluss Build"));
        this.parent = parent;
        this.cloudArt=artId;this.cloudVersion=versionId;
        var c = Minecraft.getInstance();
        anchorWorld = c.level;
        anchor = c.player == null ? null : c.player.blockPosition().immutable();
    }
    ...
    private void button(String id, String label, WorkshopIcon icon, WorkshopLayout.Rect r, java.util.function.BooleanSupplier enabled, Runnable action) {
        addRenderableWidget(MapKlussButton.builder(Component.literal(label), ignored -> {if(enabled.getAsBoolean())action.run();})
            .action(id).tooltip(Component.literal(label)).dimensions(r.x(),r.y(),r.width(),r.height())
    ...
    @Override protected void init() {
        ...
        button("tracker.build.group",tr("Друзья","Friends"),WorkshopIcon.ACCOUNT,WorkshopTrackerLayout.part(s.navigation(),3,4),
            ()->!loading&&!manager.preparing(),()->Minecraft.getInstance().gui.setScreen(new LiveBuildGroupScreen(this)));
        ...
        button("tracker.build.two_layer","Two-layer",WorkshopIcon.TRACKER,
            ...
            ()->!loading&&!manager.preparing()&&manager.canFollowTwoLayer(Minecraft.getInstance()),
            ()->{if(manager.needsTwoLayerBundle(Minecraft.getInstance())){choose(true);return;}
                try{manager.followTwoLayer(Minecraft.getInstance());error="";}catch(RuntimeException failure){error=tr("Нет активного этапа","No active stage");}});
        button("global.back",parent instanceof TrackerSessionScreen?tr("Блоки","Blocks"):tr("Назад","Back"),WorkshopIcon.BACK,
            ...
            ()->{if(parent instanceof TrackerSessionScreen tracker)tracker.showBuildMaterials();else onClose();});
    }
    ...
    private void loadCloud(){
        ...
        if(anchor==null||client.level!=anchorWorld){
        ...
        CompletableFuture.runAsync(()->{
            try{
                var linked=TrackerCloudAssets.linkedSource(CompanionRuntime.create(client).apiClient(),
                    LiveBuildSourceCache.forRunDir(client.gameDirectory.toPath()),cloudArt,cloudVersion);
                var loaded=linked.loaded();
                client.execute(()->{
                    if(request!=token||client.level!=anchorWorld){loaded.close();if(request==token)loading=false;return;}
                    loading=false;
            ...
            try {
                ...
                try(MemoryStack stack=MemoryStack.stackPush()){
                    ...
                    selected=TinyFileDialogs.tinyfd_openFileDialog(twoLayerOnly?"MapKluss Two-layer ZIP":"MapKluss Litematic",
                        LitematicaPaths.defaultSchematicDir(Minecraft.getInstance().gameDirectory.toPath()).toString(),
                        filters,"Litematic / Two-layer ZIP",false);
                }
                if(selected==null||selected.isBlank()) { Minecraft.getInstance().execute(()->{if(request==token)loading=false;});return; }
                ...
                var loaded=LiveBuildSourceCache.forRunDir(Minecraft.getInstance().gameDirectory.toPath()).importFile(path,kind);
                ...
                Minecraft.getInstance().execute(()->{
                    ...
                    loading=false;
                    try {
                        if(Minecraft.getInstance().level!=anchorWorld||anchor==null)throw new IllegalStateException("World changed");
                        if(twoLayerOnly){
                            var active=SuppressionManager.instance().trackerSession(Minecraft.getInstance());
                        ...
                        manager.openCached(Minecraft.getInstance(),loaded,anchor,null);
                ...
                Minecraft.getInstance().execute(()->{if(request==token){loading=false;error=failure instanceof LiveBuildSchematic.PhasePlanRequired
    ...
    private void start() {
        try {
            if (Minecraft.getInstance().level != anchorWorld) throw new IllegalStateException("World changed");
            Minecraft.getInstance().gui.setScreen(new LiveBuildPlacementScreen(this));error="";
        }catch(Exception failure){error=tr("Схема несовместима или мир закрыт","Incompatible schematic or unavailable world");}
    }
    private void text(GuiGraphicsExtractor g,String value,WorkshopLayout.Rect r,String colour) {
        WorkshopDraw.text(g,font,value,r.x()+4,r.y()+4,Math.max(0,r.width()-8),theme.color(colour));
    }
