...
class MapKlussButton extends ClickableWidget {
    ...
    protected void renderWidget(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        context.drawCenteredTextWithShadow(renderer, MapKlussText.text(clipped), x + getWidth() / 2, y + (getHeight() - renderer.fontHeight) / 2 + 1, textColor);
    }

    @Override
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        if (!active || !visible || !visibleWhen.getAsBoolean() || !enabledWhen.getAsBoolean()
            || button != 0 || !isMouseOver(mouseX, mouseY)) return false;
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        ...
        if (keyCode != KEY_ENTER && keyCode != KEY_NUMPAD_ENTER && keyCode != KEY_SPACE) return false;
        playDownSound(MinecraftClient.getInstance().getSoundManager());
        workshopPressedUntil = System.nanoTime() + 120_000_000L;
        onPress.onPress(this);
        return true;
