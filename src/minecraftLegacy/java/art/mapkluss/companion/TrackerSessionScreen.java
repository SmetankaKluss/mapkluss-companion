...
public final class TrackerSessionScreen extends WorkshopTrackerScreen {
    ...
    private void text(DrawContext g,String value,WorkshopLayout.Rect r,String color) {
        ...
        WorkshopDraw.text(g,textRenderer,CompanionI18n.translate(value),r.x()+4,r.y()+4,Math.max(0,r.width()-8),workshopTheme.color(color));
    }
    ...
    private void rebuildLayout(){
        if(searchInput!=null)pendingSearch=searchInput.getText();
        clearChildren();
        searchInput=null;
        ...
        if(loadFailed){
            ...
            trackerButton("tracker.change_session","Изменить UUID",WorkshopIcon.BACK,part(s.modes(),1,2),()->true,false,this::close);
            return;
        }
        ...
        trackerButton("tracker.build.open","Стройка",WorkshopIcon.TRACKER,part(s.modes(),1,4),()->session!=null,false,
            ()->client().setScreen(new LiveBuildScreen(this,session.art_id(),session.art_version_id())));
    ...
    private void addTools(WorkshopTrackerLayout.Layout s){
        var a=s.table();
        searchInput=new TextFieldWidget(textRenderer,a.x(),a.y(),a.width()-84,20,CompanionI18n.text("Поиск материалов"));
        searchInput.setMaxLength(80);searchInput.setText(pendingSearch);addDrawableChild(searchInput);
    ...
    private void rebuildMaterialButtons(){
        ...
        for(int row=0;row<s.rows()&&row+scrollOffset<materials.size();row++){
            ...
            TextFieldWidget input=new TextFieldWidget(textRenderer,r.x(),r.y(),fieldWidth,18,CompanionI18n.text("Количество"));
            input.setMaxLength(7);
            input.setTooltip(net.minecraft.client.gui.tooltip.Tooltip.of(Text.literal(materialLabel(material)+" / "+material.count())));
            input.setTextPredicate(value->value==null||value.matches("\\d{0,7}"));
            input.setText(automaticBuilding()?Integer.toString(currentProgress().getOrDefault(material.nbtName(),0)):
            ...
            input.setChangedListener(value->manualDrafts.put(draftKey(material),value));
            progressInputs.add(new ProgressInput(material,input));addDrawableChild(input);
            int x=r.x()+fieldWidth+gap;
            trackerButton("tracker.set_count","Применить",WorkshopIcon.CHECK,new WorkshopLayout.Rect(x,r.y(),bw,18),()->!automaticBuilding(),false,()->applyManualProgress(material,input.getText()));
    ...
    private void changeProgress(BuildSessionMaterial material, int delta) {
        ...
        if (session == null||automaticBuilding()) return;
    ...
    @Override public void render(DrawContext g,int mouseX,int mouseY,float delta){
        ...
        if(!toolsVisible&&showGatherPreview()&&!loadFailed){
            ...
            for(int i=0;i<s.rows()&&i+scrollOffset<materials.size();i++){
                ...
                g.drawItem(CompanionMaterialIcons.stackFor(m),r.x()+2,r.y()+1);
    ...
    @Override protected boolean submitFocusedInput(){
        ...
        for(var input:progressInputs)if(input.widget().isFocused()){
            applyManualProgress(input.material(),input.widget().getText());return true;
        }
        return false;
    }
    ...
    private void openDestination(CompanionUiLayout.Destination destination) {
        switch (destination) {
            ...
            case TRACKER -> client().setScreen(new TrackerOpenScreen(parent, fixture));
