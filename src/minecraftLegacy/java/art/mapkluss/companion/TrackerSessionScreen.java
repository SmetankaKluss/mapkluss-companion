...
public final class TrackerSessionScreen extends Screen {
    ...
    private void rebuildLayout() {
        ...
        searchInput = new TextFieldWidget(textRenderer, left, searchY, searchWidth, 22, CompanionI18n.text("Поиск материалов"));
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        int left = work.x();
        if (loadFailed) {
            MapKlussUi.drawEmptyState(context, textRenderer, "Сессия не найдена", "Проверьте UUID и повторите", left, work.y() + 30, panelWidth, Math.max(44, work.height() - 80));
        ...
        if (session != null) {
            String titleLine = session.info() != null && session.info().title() != null && !session.info().title().isBlank()
                ? session.info().title()
                : sessionId;
            MapKlussUi.drawLeft(context, textRenderer, titleLine, left, work.y() + 2, panelWidth, MapKlussUi.WHITE);
