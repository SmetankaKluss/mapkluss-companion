...
final class CompanionAccountScreen extends Screen {
    ...
    protected void init() {
        ...
        addDrawableChild(account.build());
        addDrawableChild(MapKlussButton.builder(CompanionI18n.text("Сайт облака"), button -> openSite("/cloud"))
            .action("account.open_site")
            .technical().dimensions(x + buttonWidth + gap, y, buttonWidth, 22).build());
        addDrawableChild(MapKlussButton.builder(CompanionI18n.text("Синхронизация"), button -> client().setScreen(new CompanionLibraryScreen(this, true)))
            .action("account.sync")
            .enabledWhen(this::isSignedIn).dimensions(x, y + 30, buttonWidth, 22).build());
        addDrawableChild(MapKlussButton.builder(CompanionI18n.text("Проверить обновление"), button -> checkForUpdate())
        ...
        addDrawableChild(MapKlussButton.builder(CompanionI18n.text(telemetryEnabled ? "Анонимная статистика: вкл." : "Анонимная статистика: выкл."), button -> toggleTelemetry())
            .action("account.telemetry")
            .selected(telemetryEnabled).dimensions(x, y + 60, cardWidth, 20).build());
        addDrawableChild(MapKlussButton.builder(CompanionI18n.text(detailsVisible ? "Скрыть детали" : "Детали"), button -> {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        MapKlussUi.drawLeft(context, textRenderer, CompanionI18n.translate(isSignedIn() ? "Cloud подключён" : "Вход не выполнен"), x + 14, y + 12, cardWidth - 28, MapKlussUi.WHITE);
        ...
        if (detailsVisible && session != null) {
            ...
            if (detailsY + 39 <= content.bottom() - 8) {
                MapKlussUi.drawLeft(context, textRenderer, "ID: " + session.shortUserId(), x + 14, detailsY, cardWidth - 28, MapKlussUi.MUTED);
                MapKlussUi.drawLeft(context, textRenderer, CompanionI18n.translate("Сохранено: ") + safe(session.savedAt()), x + 14, detailsY + 15, cardWidth - 28, MapKlussUi.DIM);
                MapKlussUi.drawLeft(context, textRenderer, CompanionI18n.translate("Истекает: ") + safe(session.expiresAt()), x + 14, detailsY + 30, cardWidth - 28, MapKlussUi.DIM);
            }
        }
