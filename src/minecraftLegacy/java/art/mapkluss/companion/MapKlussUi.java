...
final class MapKlussUi {
    ...
    static CompanionUiLayout.Shell drawShell(
        DrawContext context, TextRenderer renderer, int screenWidth, int screenHeight,
    ...
    static void drawNavigation(DrawContext context, CompanionUiLayout.Shell shell, CompanionUiLayout.Destination current) {
    ...
    static void drawIcon(DrawContext context, MapKlussIcon icon, int x, int y, int color) {
    ...
    static void drawHeader(DrawContext context, TextRenderer renderer, String title, String subtitle, int screenWidth, int y) {
        ...
        context.drawTextWithShadow(renderer, MapKlussText.text(clippedTitle), left + 18, y, WHITE);
    ...
    static void drawLocalHeader(
        ...
        context.drawTextWithShadow(renderer, MapKlussText.text(clippedTitle), left + 16, y, WHITE);
        if (subtitle != null && !subtitle.isBlank()) {
    ...
    static void drawRight(DrawContext context, TextRenderer renderer, String value, int right, int y, int maxWidth, int color) {
        ...
        Text text = MapKlussText.text(clipped);
        context.drawTextWithShadow(renderer, text, right - renderer.getWidth(text), y, color);
    }
    ...
    static void drawWrappedCentered(
        ...
        for (int line = 0; line < maxLines && !remaining.isEmpty(); line++) {
            ...
            context.drawCenteredTextWithShadow(renderer, MapKlussText.text(next.text()), screenWidth / 2, y + line * 11, color);
    ...
    static void drawWrappedCenteredIn(
        ...
        for (int line = 0; line < maxLines && !remaining.isEmpty(); line++) {
            ...
            WrappedLine next = takeLine(renderer, remaining, maxWidth, lastLine);
            if (next.text().isEmpty() || next.consumedChars() <= 0) return;
            context.drawCenteredTextWithShadow(renderer, MapKlussText.text(next.text()), centerX, y + line * 11, color);
            remaining = remaining.substring(Math.min(remaining.length(), next.consumedChars())).trim();
        }
    }
