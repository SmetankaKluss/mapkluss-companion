...
public record AutoFramePlacement(
    ...
    public BlockPos blockAt(int column, int rowFromBottom) {
        ...
        return leftBottom.add(
            right.getOffsetX() * column + planeUp.getOffsetX() * rowFromBottom,
            right.getOffsetY() * column + planeUp.getOffsetY() * rowFromBottom,
            right.getOffsetZ() * column + planeUp.getOffsetZ() * rowFromBottom
        );
    }
    ...
    static Direction rightDirection(Direction facing, Direction planeUp) {
        if (facing.getAxis().isHorizontal()) return facing.rotateYCounterclockwise();
        return facing == Direction.UP
            ? planeUp.rotateYClockwise()
            : planeUp.rotateYCounterclockwise();
    }
    ...
    private static int project(int x, int y, int z, Direction direction) {
        return x * direction.getOffsetX() + y * direction.getOffsetY() + z * direction.getOffsetZ();
    }
