...
public final class LensScreen extends Screen {
    ...
    private void rebuildControls() {
        ...
        lensButton("global.back", "Назад", WorkshopIcon.BACK,
            new WorkshopLayout.Rect(s.navigation().right() - 24, s.navigation().y(), 24, 28), true, false, true, this::close);
        ...
        if (s.split() || !showPreview) {
            var join = s.join();
            codeInput = new TextFieldWidget(textRenderer, join.x(), join.y(), join.width() - 32, 24, CompanionI18n.text("Код Lens"));
            codeInput.setMaxLength(20);
            codeInput.setPlaceholder(CompanionI18n.text("Код Lens"));
            codeInput.setText(code);
            codeInput.setChangedListener(value -> code = value);
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        WorkshopDraw.text(context, textRenderer, summary, s.status().x() + 4, s.status().y() + 5,
        ...
        if (s.split() || showPreview) {
            ...
            WorkshopDraw.text(context, textRenderer, title, s.metadata().x() + 4, s.metadata().y() + 6,
        ...
        if ((s.split() || !showPreview) && (placementsTab ? placements().isEmpty() : sessions().isEmpty()))
            WorkshopDraw.text(context, textRenderer, CompanionI18n.translate(placementsTab ? "Размещений пока нет" : "Нет активных сессий"),
        ...
        WorkshopDraw.text(context, textRenderer, CompanionI18n.translate(footer), s.footer().x() + 4,
    ...
    private void drawPreview(DrawContext context, WorkshopLayout.Rect p, LensDtos.Session session) {
        if (fixture && session != null) {
            var texture = net.minecraft.util.Identifier.of(MapKlussCompanionClient.MOD_ID, "textures/dev/library/great-wave.png");
