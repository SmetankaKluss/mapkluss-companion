...
public final class LensScreen extends Screen {
    ...
    private void rebuildControls() {
        ...
        codeInput = new TextFieldWidget(textRenderer, left, INPUT_Y, codeWidth, 20, CompanionI18n.text("Код Lens"));
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        if (!placementsTab && manager.sessions().isEmpty()) {
            MapKlussUi.drawEmptyState(context, textRenderer, "Нет активных сессий", "", left, LIST_Y, panelWidth, Math.max(42, height - LIST_Y - 112));
        }
        if (placementsTab && manager.placements().isEmpty()) {
            MapKlussUi.drawEmptyState(context, textRenderer, "Размещений пока нет", "", left, LIST_Y, panelWidth, Math.max(42, height - LIST_Y - 92));
        }
        MapKlussUi.drawStatusIn(context, textRenderer, manager.status(), left + panelWidth / 2, 48, panelWidth - 90);
