...
public final class TrackerOpenScreen extends WorkshopTrackerScreen {
    ...
    @Override protected void init() {
        if(sessionInput!=null)pendingSession=sessionInput.getText();
        ...
        trackerButton("tracker.build.open", CompanionI18n.english(client()) ? "Build" : "Стройка", WorkshopIcon.TRACKER,
            ...
            ()->client().setScreen(new LiveBuildScreen(this)));
        var t=s.title();
        sessionInput=new TextFieldWidget(textRenderer,t.x(),t.y(),t.width()-60,20,CompanionI18n.text("UUID сборки"));
        sessionInput.setPlaceholder(CompanionI18n.text("UUID сборки"));
        sessionInput.setMaxLength(64);
        sessionInput.setText(pendingSession);
        addDrawableChild(sessionInput);
        trackerButton("tracker.open_session","Открыть",WorkshopIcon.TRACKER,
            new WorkshopLayout.Rect(t.right()-56,t.y(),56,20),()->!sessionInput.getText().isBlank(),false,
            ()->openSession(sessionInput.getText().trim()));
    ...
    @Override protected boolean submitFocusedInput() {
        ...
        openSession(sessionInput.getText()); return true;
    }
    ...
    private void openSession(String sessionId) {
        ...
        client().setScreen(new TrackerSessionScreen(this, trimmed, fixture));
    }
